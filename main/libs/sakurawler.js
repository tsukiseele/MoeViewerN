...
export default class Sakurawler {
  ...
  async parseRules(index, rule, nextUrl = null, page = this.page, keywords = this.keywords) {
    ...
    // 检查中断
    if (this.abortController && this.abortController.isAbort) return
    // 检查无效响应
    if (!html) return []
    // 加载文档
  ...
  replaceRegex(text, capture, replacement) {
    ...
    new RegExp(capture).exec(text)
      .forEach((item, index) => (replacement = replacement.replace(new RegExp('\\$' + index, 'g'), item)))
    return replacement
  }
